...
public sealed class UserLogger : IUserLogger
{
    ...
    public void SourceElementLocated(string elementType, string sourceHandle)
    {
        var displayType = string.IsNullOrWhiteSpace(elementType) ? "源构件" : elementType;
        WriteToCommandLine($"已定位到源构件（{displayType}）");
        WriteToFile("INFO", "Located source element", new { elementType = displayType, handle = sourceHandle });
    }

    public void SourceElementLocationFailed(string reason)
    {
        WriteToCommandLine($"[错误] {reason}");
        WriteToFile("ERROR", $"Locate source element failed: {reason}");
    }

    public void RelatedSectionsFound(string sourceHandle, int count)
    {
        WriteToCommandLine($"共找到 {count} 个剖面引用了该构件");
        WriteToFile("INFO", "Related sections found", new { handle = sourceHandle, count });
    }

    public void NoRelatedSectionsFound(string sourceHandle)
    {
        WriteToCommandLine("未找到关联的剖面");
        WriteToFile("INFO", "No related sections found", new { handle = sourceHandle });
    }
    ...
    private void WriteToFile(string level, string msg, object? extra = null)
    {
        ...
        _fileProcessor.Enqueue(JsonSerializer.Serialize(entry));
    }
}
