...
public sealed class MultiFloorSectionComposer
{
        ...
        IReadOnlyList<FloorConfig> floors)
    {
        ...
        foreach (var floor in floors)
        {
            ...
            // 2. 计算对齐变换（若配置了三点对齐）
            var alignedSectionLine = ApplyAlignment(sectionLine, floor);
    ...
    /// 若楼层未配置对齐点，则直接返回原剖切线
    /// </summary>
    private static Line3D ApplyAlignment(Line3D sectionLine, FloorConfig floor)
    {
        ...
        var newStart = transform.Transform(sectionLine.Start);
        var newEnd   = transform.Transform(sectionLine.End);
