...
public sealed class LayerBasedElementRecognizer : IElementRecognizer
{
    ...
    public IReadOnlyList<BuildingElement> RecognizeElements(Line3D sectionLine, double viewDepth)
    {
        ...
        foreach (var objId in btr)
        {
            ...
            var element = ConvertToElement(entity, elementType, tr);
            if (element != null)
            {
                result.Add(element);
        ...
        tr.Commit();
        _logger.LogDebug("扫描 {ScannedCount} 个实体，识别到 {ElementCount} 个构件",
    ...
    private static BuildingElement? ConvertToElement(Entity entity, string elementType, Transaction tr)
    {
        try
        {
