...
- 图层前缀必须是 `MK_结构墙` / `MK_结构柱` / `MK_楼板`
- 当前仅支持 `Line -> 墙`、`Circle -> 柱`、`Polyline -> 楼板`
- 剖切线必须真正穿过目标构件；仅在支持图层附近但未相交的图元不会生成剖面主体

`GenSection` 结果含义：

- `Success`：所有参与楼层都正常生成
- `PartialSuccess`：剖面已生成，但存在告警，例如某些楼层无构件被跳过
- `Failed`：流程在某个阶段被阻塞，例如所有楼层都未识别到可剖切构件
...
- 检查构件实体类型是否分别为 `Line` / `Circle` / `Polyline`
- 检查楼层对齐点是否设置错误
- 若命令提示 `PartialSuccess`，说明至少一个楼层生成成功，但仍有楼层被跳过，请结合用户日志查看具体楼层和原因
